...
| 02/08 | Premier titre du jour affiché « ? » dans le rapport (14 fois du 30/06 au 01/08). Cause : pendant le restart de minuit, l'AutoDJ est indisponible ~1 s, Liquidsoap bascule sur sa source de repli et joue un jingle de `000_TRANSITION` **hors file AutoDJ** — AzuraCast ne reçoit aucune métadonnée (son propre now-playing est vide, cf. `midnight_watch`) | `playback_report.py` reconstitue le titre depuis la ligne `Prepared "…"` du log Liquidsoap la plus récente **≤** l'heure de diffusion (fenêtre 120 s). Règle validée sur 2 130 titres connus des rapports du 22/07 au 01/08 (97 % retrouvés) ; les 6 titres inconnus de la période sont tous identifiés. La provenance est affichée — un titre reconstitué ne se fait pas passer pour une métadonnée d'origine — et le titre reste compté en « bouche-trou » |
...
6. **Un titre s'affiche « ? »** → la ligne `IDENTIFICATION` du MONITEUR dit pourquoi. Titre *reconstitué* = AzuraCast n'a pas reçu la métadonnée (passage hors file AutoDJ) mais le log Liquidsoap avait la réponse. Titre *non identifiable* = l'archive `liquidsoap_events_<date>.log` manque pour ce jour (capture horaire en place seulement depuis le 18/07/2026) : vérifier le workflow `radio-liquidsoap-capture`. Un rapport ancien ne se corrige pas tout seul — relancer `radio-playback-report` avec la date voulue pour le régénérer.
